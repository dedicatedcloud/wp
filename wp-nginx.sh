...
#sudo apt-add-repository ppa:ondrej/nginx-mainline -y
#sudo apt-add-repository ppa:ondrej/php -y

sudo apt-key adv --fetch-keys 'https://mariadb.org/mariadb_release_signing_key.asc'
sudo add-apt-repository 'deb [arch=amd64,arm64,ppc64el] http://mirror.netinch.com/pub/mariadb/repo/10.4/ubuntu focal main'

sudo apt update && sudo apt install -y build-essential curl nano wget lftp unzip bzip2 arj nomarch lzop htop openssl gcc git binutils libmcrypt4 libpcre3-dev make python3 python3-pip supervisor unattended-upgrades whois zsh imagemagick uuid-runtime net-tools
...
sed -i "s/upload_max_filesize = .*/upload_max_filesize = 500M/" /etc/php/7.4/fpm/php.ini
sed -i "s/post_max_size = .*/post_max_size = 500M/" /etc/php/7.4/fpm/php.ini
